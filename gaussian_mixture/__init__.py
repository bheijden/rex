...
def plot_component_norm_pdfs(
    ...
    for component in range(pdfs.shape[1]):
        ax.plot(x, pdfs[:, component], label=f"Component {component}")
    ax.plot(x, pdf, linestyle="--", label="Mixture", color="red")
    ax.set_title(title)
...
def animate_training(params_for_plotting, data_mixture, num_frames, fig=None, ax=None, edgecolor: str = None, facecolor: str = None, bins: int = 40, xmin: float = None, xmax: float = None, num_points: int = 1000):
    ...
    try:
        ...
        print(f"Please install celluloid to animate training: {e}")
    ...
    for w, m, s in tqdm(zip(
        log_component_weights_history[::interval],
        component_mus_history[::interval],
        log_component_scales_history[::interval],
    ), total=len(log_component_weights_history[::interval])):
        sns.histplot(data_mixture, bins=bins, ax=ax, stat="density", label="data", edgecolor=edgecolor, facecolor=facecolor)
        plot_component_norm_pdfs(w, m, s, xmin=xmin, xmax=xmax, ax=ax, num_points=num_points, title=None)
        cam.snap()
...
class GMMEstimator:
    ...
    def fit(self, num_steps: int, num_components: int, step_size: float = 0.05, seed: int = 0):
        ...
        print(f"TRAIN | Time taken: {time() - start:.2f} seconds.")
    ...
    def plot_hist(self, ax: plt.Axes = None, edgecolor: str = None, facecolor: str = None, bins=100, xmin: float = None, xmax: float = None, num_points: int = 1000):
        if ax is None:
            fig, ax = plt.subplots()
        sns.histplot(self.data, ax=ax, bins=bins, stat="density", label="data", edgecolor=edgecolor, facecolor=facecolor)
        if self.final_state_norm is not None:
    ...
    def animate_training(self, num_frames: int = 30, fig: plt.Figure = None, ax: plt.Axes = None,
                         ...
                         xmin: float = None, xmax: float = None, num_points: int = 1000) -> matplotlib.animation.ArtistAnimation:
        ...
        animation = animate_training(params_for_plotting, self.data, num_frames, fig=fig, ax=ax, edgecolor=edgecolor,
                                     facecolor=facecolor, bins=bins, xmin=xmin, xmax=xmax, num_points=num_points)
        return animation

    def get_truncated_gmm(self, percentile: float = 0.01):
        # todo: Truncate the gmm based on weights --> then percentile?
