...
# Custom changes
from experiments.exp_manager import ExperimentManager


def train() -> None:
	parser = argparse.ArgumentParser()
	parser.add_argument("--algo", help="RL Algorithm", default="sac", type=str, required=False, choices=list(ALGOS.keys()))
	parser.add_argument("--env", type=str, default="RecordEnv-v0", help="environment ID")
	parser.add_argument("--eval-env", type=str, default=None, help="environment ID of evaluation environment")
	parser.add_argument(
		"--gym-packages",
		type=str,
		nargs="+",
	...
	parser.add_argument(
		"--eval-env-kwargs", type=str, nargs="+", action=StoreDict, help="Optional keyword argument to pass to the eval env constructor",
		default=None,
	)
	...
	eval_env_id = env_id if args.eval_env is None else args.eval_env
	eval_env_kwargs = deepcopy(args.env_kwargs) if args.eval_env is None else args.eval_env_kwargs
	eval_vec_env = args.vec_env if args.eval_vec_env is None else args.eval_vec_env

	# If the environment is not found, suggest the closest match
	for _env_id in [env_id, eval_env_id]:
		if _env_id not in registered_envs:
			try:
				closest_match = difflib.get_close_matches(_env_id, registered_envs, n=1)[0]
			except IndexError:
				closest_match = "'no close match found...'"
			raise ValueError(f"{_env_id} not found in gym registry, you maybe meant {closest_match}?")
	...
	exp_manager = ExperimentManager(
		...
		eval_vec_env_type=eval_vec_env,
...
if __name__ == "__main__":
	...
	if len(args) == 1:
		args += ["--algo", "sac",
		         "--n-timesteps", "100000",
		         "--gym-packages", "experiments",
		         "--env", "RecordEnv-v0",
		         # "--eval-env", "RecordEnv-v0",
		         "--env-kwargs", "file: '/home/r2ci/rex/logs/dp_8torque_delay_3obs_2act_more_weight_2023-01-30-1136/double_pendulum_ode_train_buffer_phase_awin2_owin3_blocking_noadvance_compiled_vectorized.pkl'",
		         "--eval-env-kwargs", "file: '/home/r2ci/rex/logs/dp_8torque_delay_3obs_2act_more_weight_2023-01-30-1136/double_pendulum_ode_eval_buffer_phase_awin2_owin3_blocking_noadvance.pkl'",
		         "--vec-env", "rex",
		         "--eval-vec-env", "rex",
		         "-optimize",
		         "--n-trials", "100",
