...
from experiments import RexVecEnv
...
class ExperimentManager:
    ...
    def __init__(
        ...
        self.vec_env_class = {"dummy": DummyVecEnv, "subproc": SubprocVecEnv, "rex": RexVecEnv}[vec_env_type]
        self.eval_vec_env_class = {"dummy": DummyVecEnv, "subproc": SubprocVecEnv, "rex": RexVecEnv}[eval_vec_env_type]
    ...
    def create_envs(self, n_envs: int, eval_env: bool = False, no_log: bool = False) -> VecEnv:
        ...
        """
        env_name = self.env_name if not eval_env else self.eval_env_name
        env_kwargs = self.env_kwargs if not eval_env else self.eval_env_kwargs
        vec_env_class = self.vec_env_class if not eval_env else self.eval_vec_env_class
        # vec_env_kwargs = self.vec_env_kwargs if not eval_env else self.eval_vec_env_kwargs
        ...
        if (
            "Neck" in env_name.gym_id
            or self.is_robotics_env(env_name.gym_id)
            or "parking-v0" in env_name.gym_id
        ...
        if self.vec_env_class == RexVecEnv:
            env = RexVecEnv(make_env,
                            n_envs=n_envs,
                            seed=self.seed,
