...
from rex.base import CompiledGraphState, GraphState, Empty, StepState, Output, GraphBuffer, SeqsMapping
...
def single_loss(env: BaseEnv, graph_state: GraphState, seqs_step: SeqsMapping, params: optax.Params, outputs: Dict[str, Output],
                rng: jnp.array, starting_eps: Union[int, jax.typing.ArrayLike], starting_step: Union[int, jax.typing.ArrayLike], num_steps: Union[int, jax.typing.ArrayLike]) -> Tuple[jax.Array, GraphBuffer]:
	...
	eps = jnp.clip(starting_eps, 0, max_eps - 1)
	step = jnp.clip(starting_step, 0, max_step - 1)
	eps_timings = rjax.tree_take(graph_state.timings, eps)

	def _fill_buffer(seqs, arr):
		exp_dims = [1] * (arr.ndim - 1)
		exp_seqs = jnp.expand_dims(seqs, exp_dims)
		return jnp.take_along_axis(arr, exp_seqs, axis=0)

	# Fill buffer with outputs
	new_outputs = {}
	eps_outputs = rjax.tree_take(outputs, eps, axis=0)
	for output_name, b in graph_state.buffer.outputs.items():
		bsize = seqs_step[output_name].shape[-1]
		seqs = jax.lax.dynamic_slice(seqs_step[output_name], [eps, step, 0], [1, 1, bsize])[0, 0]
	...
	def _step(carry, x):
		...
		done = jnp.logical_or(terminated, truncated)  # todo: correct?
	...
	carry_out, scan_out = jax.lax.scan(_step, (gs, loss_init, loss_init.loss), None, length=num_steps)  # todo: unroll?
...
def fit(env: BaseEnv, params: optax.Params, optimizer: optax.GradientTransformation, graph_state: GraphState, outputs: Dict[str, Output], num_batches: int = 20, num_steps: int = 3, num_training_steps: int = 1000,
        num_training_steps_per_epoch: int = 100, prior_fn: Callable[[optax.Params], Any] = lambda x: Union[float, jax.typing.ArrayLike](0),
        callbacks: Dict[str, "BaseCallback"] = None) -> optax.Params:
	max_eps, max_steps = next(iter(graph_state.timings[-1].values()))["run"].shape
	start_eps, start_steps = jnp.meshgrid(jnp.arange(0, max_eps), jnp.arange(0, max_steps))
	...
	# loss, new_buffer = single_loss(env, graph_state, seqs_step, params, outputs, jax.random.PRNGKey(0), 0,
	...
	def loss(params: optax.Params, _outputs: Dict[str, Output], _rng: jax.random.KeyArray) -> Tuple[jnp.ndarray, Dict[str, Output]]:
		...
		steps = jnp.take(start_steps, choice)
		eps = jnp.take(start_eps, choice)

		# Perform batch rollout
		loss_batch, buffer_batch = batch_loss(params, _outputs, _rngs, eps, steps, num_steps)

		# Add prior loss on params
		ploss = prior_fn(params)
		ploss = jax.tree_util.tree_reduce(lambda acc, l: acc + jnp.sum(l), ploss, 0.)
	...
	# loss_batch, new_outputs = loss(params, outputs, jax.random.PRNGKey(0))
	...
	def train_step(carry: Carry, _rng):
		...
		params["world"] = jax.tree_util.tree_map(lambda x: jnp.abs(x), params["world"])
	...
	# Prepare config
	config = Config(epoch=0, env=env, optimizer=optimizer, opt_state=opt_state, params=params, graph_state=graph_state,
	...
	# Run training epochs
	rngs_epoch = jax.random.split(jax.random.PRNGKey(0), num=num_epochs)
	for i, rng in enumerate(rngs_epoch):
		...
		# Run epoch
		rngs_step = jax.random.split(rng, num=num_training_steps_per_epoch)
		...
		# with timer:
		# todo: why jax.lax.scan only works with jit?
		# carry, scan_out = jax.lax.scan(train_step, carry, rngs_step, length=num_training_steps_per_epoch, unroll=1)
...
def plot_trajectory(targets: jax.typing.ArrayLike = None, preds_hidden: jax.typing.ArrayLike = None, label: str = None, ax: plt.Axes = None, size: float = 2):
	label = label or ""
	if ax is None:
		fig, ax = plt.subplots()
...
def visualize(_world_states, nodes, ts_actuator, ts_sensor, ts_world):
	...
	targets = jnp.stack((ts_sensor, nodes["sensor"]._outputs.cos_th))
	preds_hidden = jnp.stack((ts_world, jnp.cos(_world_states.th)))
	_, art_cos_th = plot_trajectory(targets, preds_hidden, label="cos(th)", ax=axes[0])

	targets = jnp.stack((ts_sensor, nodes["sensor"]._outputs.cos_th2))
	preds_hidden = jnp.stack((ts_world, jnp.cos(_world_states.th2)))
	_, art_cos_th2 = plot_trajectory(targets, preds_hidden, label="cos(th2)", ax=axes[1])

	targets = jnp.stack((ts_sensor, nodes["sensor"]._outputs.thdot))
	preds_hidden = jnp.stack((ts_world, _world_states.thdot))
	_, art_thdot = plot_trajectory(targets, preds_hidden, label="thdot", ax=axes[2])

	targets = jnp.stack((ts_sensor, nodes["sensor"]._outputs.thdot2))
	preds_hidden = jnp.stack((ts_world, _world_states.thdot2))
	_, art_thdot2 = plot_trajectory(targets, preds_hidden, label="thdot2", ax=axes[3])

	# targets = jnp.stack((ts_actuator[:gs.outputs["actuator"].action.shape[0]], gs.outputs["actuator"].action[:, 0]))
	targets = jnp.stack((ts_actuator, nodes["actuator"]._outputs.action[:, 0]))
...
def init_graph_state(env: BaseEnv, nodes: Dict[str, Node], record: log_pb2.NetworkRecord, MCS: nx.DiGraph, Gs: List[nx.DiGraph], Gs_monomorphism: List[Dict[str, Tuple[int, str]]], data: exp.RecordHelper):
	...
	def _update_buffer(_data: jax.typing.ArrayLike, _buffer: jax.typing.ArrayLike) -> jax.Array:
	...
	params = nodes["sensor"].default_params(jax.random.PRNGKey(0))
	params = params.replace(th_std=0., th2_std=0., thdot_std=0., thdot2_std=0.)
	...
	for eps in range(num_eps):
		world_outputs = rjax.tree_take(outputs["world"], eps, axis=0)
		o = jax.tree_util.tree_map(partial(_fill_buffer, seqs_world[eps]), world_outputs)
		world_states.append(o)
	world_states = jax.tree_util.tree_map(lambda *x: jnp.stack(x, axis=0), *world_states)

	# Define initial estimator params
	# num_steps = env._cgraph.max_steps + 1
	# world_state = nodes["world"].default_state(jax.random.PRNGKey(0))
	# world_states = jax.tree_map(lambda *x: jp.repeat(jnp.stack(x)[None], num_eps, axis=0), *([world_state] * (num_steps+1)))
	...
	init_gs = CompiledGraphState(nodes=FrozenDict(ndict), timings=timings, buffer=buffer)
	init_gs, _, _ = env.reset(jax.random.PRNGKey(0), init_gs)
...
def _init_progress(record: log_pb2.EpisodeRecord, nodes: Dict[str, Node], graph_state: GraphState, wseqs):
	...
	# Get ts
	ts_actuator = jnp.array([s.ts_output for s in record_actuator.steps])
	ts_sensor = jnp.array([s.ts_step for s in record_sensor.steps])
	ts_estimator = jnp.array([s.ts_step for s in record_est.steps])
	ts_world = jnp.array(
	...
	def progress_fn(metrics, carry, scan_out):
		...
		art_hidden.set_offsets(jnp.stack((ts_world, jnp.cos(_world_states.th)), axis=1))
		_, art_hidden = artists["cos_th2"]
		art_hidden.set_offsets(jnp.stack((ts_world, jnp.cos(_world_states.th2)), axis=1))
		_, art_hidden = artists["thdot"]
		art_hidden.set_offsets(jnp.stack((ts_world, _world_states.thdot), axis=1))
		_, art_hidden = artists["thdot2"]
		art_hidden.set_offsets(jnp.stack((ts_world, _world_states.thdot2), axis=1))
