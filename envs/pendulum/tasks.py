        ...
        if verbose:
            class _PrettyPrint:
                ...
                def __repr__(self):
                    ...
                    return f"{self.xt} Rel({self.x})"

            opt_params_trans = self.trans.apply(opt_params)
            pp = jax.tree_util.tree_map(lambda _xt, _x: _PrettyPrint(_xt, _x), opt_params_trans, opt_params)
...
def create_control_task(graph: Graph, graph_state: base.GraphState, strategy: str = "OpenES"):
    ...
    def rollout_fn(graph: Graph, params: Dict[str, base.Params], rng: jax.Array = None) -> base.GraphState:
        ...
        graph_init = functools.partial(graph.init, order=("supervisor",))
...
def create_sysid_task(graph: Graph, graph_state: base.GraphState, strategy: str = "CMA_ES"):
    ...
    # Detection (detection)
    detector = graph.nodes["detector"]
    median = onp.array(detector._outputs.median).reshape(-1, 2)
    a, b, x0, y0, phi = detector.estimate_ellipse(median)
    print(f"Initial guess: a={a}, b={b}, x0={x0}, y0={y0}, phi={phi}")
    init_params["detector"] = base_params["detector"].replace()
    init_params["detector"] = init_params["detector"].replace(a=a, b=b, x0=x0, y0=y0, phi=phi)  # Transformation to pendulum angle
    init_params["detector"] = init_params["detector"].replace(min_max_threshold=None, lower_bgr=None, upper_bgr=None,
                                                              sigma=None, binarization_threshold=None)  # Image processing
    # init_params["detector"] = init_params["detector"].replace(a=None, b=None, x0=None, y0=None, phi=None, theta_offset=None)  # Transformation to pendulum angle
    init_params["detector"] = init_params["detector"].replace(wn=None)  # Low-pass filter
    u_min["detector"] = jax.tree_util.tree_map(lambda x: x * 0.9, init_params["detector"])
    u_max["detector"] = jax.tree_util.tree_map(lambda x: x * 1.1, init_params["detector"])
    u_min["detector"] = eqx.tree_at(lambda _min: _min.phi, u_min["detector"], -2 * onp.pi)
    u_max["detector"] = eqx.tree_at(lambda _max: _max.phi, u_max["detector"], 2 * onp.pi)
    u_min["detector"] = eqx.tree_at(lambda _min: _min.theta_offset, u_min["detector"], -2 * onp.pi)
    u_max["detector"] = eqx.tree_at(lambda _max: _max.theta_offset, u_max["detector"], 2 * onp.pi)

    # Estimator
    init_params["estimator"] = base_params["estimator"].replace(dt_future=0.0)
    ...
    u_max["estimator"] = eqx.tree_at(lambda _max: _max.dt_future, u_max["estimator"], 0.2)
    ...
    def rollout_fn(graph: Graph, params: Dict[str, base.Params], rng: jax.Array = None) -> base.GraphState:
        # Initialize graph state
        gs = graph.init(rng=rng, params=params, order=("supervisor",))
    ...
    def plot_fn(graph_states: base.GraphState, identifier: str = "") -> Any:
        ...
        thdot_world = graph_states.state["world"].thdot
        # thdot_detector = graph_states.nodes["detector"].state.yn_1
        axes[1].plot(ts_world, thdot_world, label="world")
        axes[1].plot(ts_sensor, thdot_sensor, label="sensor")
        # axes[1].plot(ts_detector, thdot_detector, label="detector")
