...
def make_sysid_pendulum_system_nodes(record: base.EpisodeRecord,
    ...
    # Create camera
    from envs.pendulum.realsense import SimD435i
    camera = SimD435i.from_info(record.nodes["camera"].info, outputs=outputs.get("camera", None))

    # Create detector
    from envs.pendulum.detector import SimDetector
    detector = SimDetector.from_info(record.nodes["detector"].info, outputs=outputs.get("detector", None))

    # Create estimator
    from envs.pendulum.estimator import Estimator
    estimator = Estimator.from_info(record.nodes["estimator"].info)

    # Create controller
    from envs.pendulum.controller import MLPController
    controller = MLPController.from_info(record.nodes["controller"].info)
    ...
    nodes = dict(sensor=sensor, camera=camera, detector=detector, estimator=estimator, controller=controller, actuator=actuator, supervisor=supervisor)
    ...
    camera_delay = base.TrainableDist.create(alpha=0., min=0.0, max=1 / camera.rate)
...
if __name__ == "__main__":
    ...
    # todo: Real pendulum:
    #   - Why does the actuator have such a big delay?
    #   - Improve graph:
    #       - merge camera & detector
    #       - blocking connections between estimator, controller, actuator
    #       - connect estimator to actuator
    #       - throttle in actuator until action.ts - actuator_delay
    ...
    outputs_sysid["camera"] = None  # Do not use images for now
    nodes_sysid = make_sysid_pendulum_system_nodes(record, outputs=outputs_sysid, world_rate=WORLD_RATE)
    nodes_ctrl = make_sysid_pendulum_system_nodes(record, outputs={}, world_rate=WORLD_RATE)
    ...
    with timer(f"warmup[graph_state]", log_level=100):
        gs = base_init(RNG, order=("supervisor",))
    # for name, node in graph_sysid.nodes.items():
    ...
    if False:
        ...
        gs = base_init(rng_init, order=("supervisor",))
        ...
        gs = graph_ctrl.init(rng_init, params=params, order=("supervisor",))
        # plt.show()

    # System identification
    if True:
        task = tasks.create_sysid_task(graph_sysid, gs).replace(max_steps=30)  # todo: remove
        # Initial guess
        init_params = task.to_extended_params(gs, task.init_params)
        ...
        # Overwrite base graph state
        gs = base_init(RNG, params=params, order=("supervisor",))
        # plt.show()
