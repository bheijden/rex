...
class Graph(BaseGraph):
    ...
    def reset(self, graph_state: GraphState) -> Tuple[GraphState, float32, Any]:
        ...
        assert len({n.eps for n in self.nodes.values()}) == 1, "All nodes must have the same episode counter."

        # Start nodes (provide same starting timestamp to every node)
        start = time.time()
        [n._start(start=start) for n in self.nodes.values()]
    ...
    def stop(self, timeout: float = None):
        ...
        # Stop all nodes
        fs = [n._stop(timeout=timeout) for n in self.nodes.values()]
