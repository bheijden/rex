...
control_high = 0.35 * 3.1416 * jnp.ones(m.act_size())  # 0.2
...
def _cost(params, state, action):
    ...
    x_i = pipeline_state.x.vmap().do(
        base.Transform.create(pos=params.link.inertia.transform.pos)
    )
...
with timer("jit[cem]", log_level=100):
    X, mean, obj = jit_cem(m, init_state, init_controls, control_low, control_high, jax.random.PRNGKey(0))
with timer("eval[cem]", log_level=100):
    _ = jit_cem(m, init_state, init_controls, control_low, control_high, jax.random.PRNGKey(0))
...
for i in pbar:
    ...
    X, mean, obj = jit_cem(m, next_state, next_controls, control_low, control_high, jax.random.PRNGKey(0))
    total_cost, info = jit_cost(m, next_state, mean[0])
