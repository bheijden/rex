...
if __name__ == "__main__":
	...
	# todo: Extract timings from graph
	# todo: How to hierarchically save graphs?
	#       1. Save the full graph (unpruned) (G_full).
	#       2. Given a root node and split, save unqiue motifs and supergraph.
	...
	def get_minimum_common_supergraph(G_motifs: List[nx.DiGraph], max_evals_per_motif: int = None, max_total_evals: int = 100_000):
		...
		G_super = as_MCS(G_motifs_max.copy(as_view=False))
		...
		for i_motif, (root_name, G_motif) in enumerate(G_motifs.items()):
			...
			prev_num_nodes = G_super.number_of_nodes()
			# Find the MCS
			num_evals_motif, G_super = MCS(G_super, G_motif, max_evals=max_evals_per_motif)
			# Update the total number of evaluations
			num_evals += num_evals_motif
			next_num_nodes = G_super.number_of_nodes()
	...
	if False:
		...
		generations = list(nx.topological_generations(G_super))
		ax.set(facecolor=oc.ccolor("gray"), xlabel="generation", yticks=[], xlim=[-1, 0.3])
		plt.show()

	if False:
		fig, ax = plt.subplots(figsize=(12, 5))
		plot_graph(G_super, ax=ax)
		generations = list(nx.topological_generations(G_super))
		ax.set(facecolor=oc.ccolor("gray"), xlabel="generation", yticks=[], xlim=[-1, len(generations)])
		plt.show()

	G_super = get_minimum_common_supergraph(G_motifs, max_total_evals=100_000)
	print(f"num_nodes={G_super.number_of_nodes()} | num_edges={G_super.number_of_edges()}")

	# Add root node slot as the first generation
	generations = next(nx.topological_generations(G_super))
	some_root = next(iter(root_nodes.keys()))
	G_super.add_node(some_root, **root_nodes[some_root])

	# Add edges between root node and first generation
	for node in generations:
		G_super.add_edge(some_root, node, **edge_data)

	# Convert to super graph
	G_super = as_MCS(G_super)

	# Verify that all subgraphs are monomorphic with the supergraph
	for root_test, G_test in G_subgraphs.items():
		matcher = isomorphism.DiGraphMatcher(G_super, G_test, node_match=node_match, edge_match=edge_match)
		assert matcher.subgraph_is_monomorphic(), "Subgraph is not monomorphic with the supergraph."

	# Get mappings from supergraph to subgraphs
