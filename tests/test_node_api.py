...
def test_node_api():
    ...
    actuator.connect(agent, blocking=True, delay=Gaussian(0.003, 0.001), skip=False, jitter=BUFFER, phase=0.05)
    ...
    try:
        agent.connect(actuator, blocking=True, delay=Gaussian(0.003, 0.001), skip=False, jitter=BUFFER, phase=0.05)
        agent.phase
