...
def test_tree_extend():
    ...
    tree_extended = jax.tree_util.tree_map(
        lambda base_x, ex_x: base_x if ex_x is None else ex_x, tree_template, tree_extended_structure
    )  # Fill in None values with base values
    assert jnp.array_equal(tree_extended["a"], jnp.array([1, 2]))
    assert jnp.array_equal(tree_extended["b"], jnp.array([3, 4, 5]))
    assert jnp.array_equal(
        tree_extended["c"], jnp.zeros((4,))
    )  # "c" was None in tree, so it should be filled with value from tree_template
